...
public class Song {
    ...
    public Song(String filePath){
        ...
        try {
            ...
            //use the jaudiotagger library to create an audio file to read mp3's files information
            AudioFile audioFile = AudioFileIO.read(new File(filePath));

            //read through thte metadata of the audio file
            Tag tag = audioFile.getTag();
            if (tag != null) {
                songTitle = tag.getFirst(FieldKey.TITLE);
                songArtist = tag.getFirst(FieldKey.ARTIST);
            } else {
                //could not read through the mp3 file's metadata
                songTitle = "N/A";
                songArtist = "N/A";
